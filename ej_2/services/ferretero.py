...
class Ferretero(RiskRequest):
    ...
    def risk_analysis(self) -> RiskResponse:
        ...
        last_bought, bought_mix, level = (
            self.input_data["last_bought"],
            self.input_data["bought_mix"],
        ...
        response = RiskResponse(
            user_id=self.user_id, product_name=self.product_name, status=status, amount=amount
        )
        return response
